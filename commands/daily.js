...
module.exports = {
    name: 'daily',
    description: 'Shows the top 10 users with the most accepted vouches for today.',
    async execute(message) {
        ...
        for (const vouch of vouchesDB) {
            if (vouch.status === 'accepted' && vouch.timestamp >= startOfDayTimestamp) {
        ...
        const leaderboardEmbed = new EmbedBuilder()
            .setColor(0x5865F2) 
            .setTitle('Rexon Daily Leaderboard')
            .setDescription(
                leaderboard.join('\n') || 
                'No accepted vouches have been recorded today. Start vouching now!'
            )
            .setFooter({
                text: `Today's leaderboard • ${new Date().toLocaleDateString()}`,
